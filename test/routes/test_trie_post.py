...
class TestCachedTriePost(TestCachedApiParent):

    def test_valid_suffix(self) -> None:
        post_data = {"suffix": "cat", "weight": 1}
        post_response = self.client.post("/api/cached-trie", data = post_data)
        self.assertEqual(post_response.status_code, 200)
        self.assertEqual(post_response.json["suffix"], "cat")
    ...
    def test_no_data(self) -> None:
        post_response = self.client.post("/api/cached-trie")
        self.assertEqual(post_response.status_code, 400)