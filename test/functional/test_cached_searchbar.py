...
class TestCachedTrieSearchBar(FunctionalTest):

    def get_suggestions(self) -> List[WebElement]:
        container = self.browser.find_element(By.ID, "cached-suggestions")
        parent = container.find_elements(By.TAG_NAME, "ul")
        return parent[0].find_elements(By.CLASS_NAME, "suggestion")  if parent else []


    def test_cached_search_bar_behavior(self) -> None:
        # Marge visits the webpage exited to try out the search bar.
        self.browser.get(self.base_url)
        searchbar = self.browser.find_element(By.ID, "trie-cached-search-bar")
        h1 = self.browser.find_element(By.TAG_NAME, "h1")
        input = self.browser.find_element(By.ID, "cached-search-input")
        suggestions_list = self.browser.find_element(By.ID, "cached-suggestions")
