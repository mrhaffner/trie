...
from app import trie, cached_trie, weighted_trie
...
def get_suggestions() -> None:
    ...
    Format: GET /api/trie/suggestions?prefix=the-prefix-you-are-looking-for
            ...
            GET /api/trie/suggestions?limit=the-maximum-amount-of-suggestions
    Response status codes:
...
def weighted_get_suggestions() -> None:
    ...
    Format: GET /api/weighted-trie/suggestions?prefix=the-prefix-you-are-looking-for
            ...
            GET /api/weighted-trie/suggestions?limit=the-maximum-amount-of-suggestions
    Response status codes:
        200: Indicates a successful retrieval (including no suggestions)
    """
    prefix_arg = request.args.get("prefix")
...
@bp.route("/api/cached-trie", methods=["GET"])
def cached_contains_suffix() -> None:
    """
    Indicates whether the Cached Trie contains the given suffix with 
        a status code
    Format: GET /api/cached-trie?suffix=the-suffix-you-are-looking-for
    Response status codes:
        200: The Cached Trie contains the suffix
        400: A suffix arguement was not given
        404: The Cached Trie does not contain the suffix
    """
    suffix_arg = request.args.get("suffix")
    if not suffix_arg:
        return "", 400
    ...
    contains_suffix = cached_trie.contains(suffix) if suffix else False
...
@bp.route("/api/cached-trie", methods=["POST"])
def cached_post_suffix() -> None:
    """
    Adds a suffix to the Cached Trie
    Adding a suffix already in the Cached Trie will overwrite its weight
    Adding a suffix with a weight < 1 will effectively delete it
    JSON POST format: 
        {"suffix": "the-suffix-you-want-to-add", "weight": integer}
    Response status codes:
        200: The suffix was added to the Cached Trie
        400: Suffix and/or weight arguement was not given
        422: The suffix was not added to the Cached Trie (invalid suffix)
    """
    suffix = request.form["suffix"] if "suffix" in request.form else None
    weight = request.form["weight"] if "weight" in request.form else None
    ...
    suffix_entry = {"suffix": suffix, "weight": weight}
    success = cached_trie.insert(suffix_entry)
...
@bp.route("/api/cached-trie", methods=["DELETE"])
def cached_delete_suffix() -> None:
    """
    Deletes a suffix from the Cached Trie
    JSON DELETE format: {"suffix": "the-suffix-you-want-to-delete"}
    Response status codes:
        204: The suffix was deleted from the Cached Trie
        400: A suffix arguement was not given
        422: The suffix was not deleted from the Cached Trie 
             (suffix invalid or not in trie)
    """
    suffix = request.form["suffix"] if "suffix" in request.form else None
    if suffix is None:
    ...
    success = cached_trie.delete(suffix)
...
@bp.route("/api/cached-trie/suggestions", methods=["GET"])
def cached_get_suggestions() -> None:
    """
    Returns all possible suffixes for a given prefix in a JSON list sorted by weight
    Format: GET /api/cached-trie/suggestions?prefix=the-prefix-you-are-looking-for
            GET /api/cached-trie/suggestions 
                (prefix = "", returns all suffixes in the Cached Trie)
            GET /api/cached-trie/suggestions?limit=the-maximum-amount-of-suggestions
    Response status codes:
        200: Indicates a successful retrieval (including no suggestions)
    """
    prefix_arg = request.args.get("prefix")
    ...
    suffixes = cached_trie.get_suffixes(prefix)
    limit_arg = request.args.get("limit")
    limit = None
