...
class TestWeightedGetSuffixes(TestCachedTrieParentWithSuffixes):

    def test_empty_trie(self) -> None:
        self.assertEqual(CachedTrie(1).get_suffixes("a"), [])


    def test_not_in_trie(self) -> None:
        self.assertEqual(CachedTrie(1).get_suffixes("zebra"), [])
...
class TestGetSuffixesFromNode(TestCachedTrieParent):
